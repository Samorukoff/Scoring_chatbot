...
def grid_search():
    ...
    # Поиск параметров для дерева в бэггинге
    base_tree_params = {
        'max_depth': [5, 7],
        'min_samples_split': [2, 10],
        'min_samples_leaf': [1, 5],
    ...
    base_tree_search = GridSearchCV(DecisionTreeClassifier(), base_tree_params, cv=3, n_jobs=1, verbose=1)
    base_tree_search.fit(X_train, y_train)
    best_tree = base_tree_search.best_estimator_
    print("Best Tree for Bagging:", base_tree_search.best_params_)

    # Бэггинг
    bag = BaggingClassifier(estimator=best_tree)
    bag_params = {
        'n_estimators': [10, 50],
        'max_samples': [0.5, 0.8],
